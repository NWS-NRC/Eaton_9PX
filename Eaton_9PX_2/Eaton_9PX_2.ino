...
void checkBattStats()
{
  ...
  if (Serial1.available() > 0)
  {
    ...
    loadPercent = int(response[4]);
    uint8_t battTemp1 = response[5];
    uint8_t battTemp2 = response[6];
    battT = int((battTemp2 << 8) | battTemp1);
    uint8_t battV1 = response[14];
    uint8_t battV2 = response[15];
    battV = float(int((battV2 << 8) | battV1) / 10);
